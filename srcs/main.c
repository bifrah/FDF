...
/*   Updated: 2021/11/03 16:59:15 by bifrah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	my_mlx_pixel_put(t_env *img_data, int x, int y, int color)
{
	...
	dst = img_data->img_ptr + (y * img_data->line_length + x * (img_data->bits_per_pixel / 8));
...
int	main(void)
{
	...
	env.img_ptr = mlx_get_data_addr(env.img_data, &env.bits_per_pixel, &env.line_length,
								&env.endian);
